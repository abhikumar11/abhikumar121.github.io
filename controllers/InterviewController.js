...
module.exports.enrollInterview = async (req, res) => {
     try {
          ...
          if (interview) {
               ...
               if (student) {
                    let exist = await Interview.findOne({
                         "students.student": student.id,
                    });
                    if (exist) {
                         if (exist.company === interview.company) {
          ...
          await interview.updateOne({$push: { students: studentobj }});
          let assign = {
               company: interview.company,
               date: interview.date,
               result: result,
             };
             await student.updateOne({$push: { interviews: assign }});
             console.log(
               "success",
